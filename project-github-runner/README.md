...
| `enableWorkloadProfiles`      | Enables workload-profile environments. When true, `platformReservedCidr`, `platformReservedDnsIp`, and `dockerBridgeCidr` are ignored per ACA rules. | Bicep parameter; see [Networking parameters](https://learn.microsoft.com/en-us/azure/container-apps/vnet-custom?tabs=workload-profiles-env#networking-parameters) |
